...
def test_set_hide_def_updates_the_kernel_baseline_and_disk(tmp_path):
    from codeslides.loader import load_deck
    ...
    cell = kernel.set_hide_def(session, "live_demo", True)

    assert cell.hide_def is True
    assert kernel.deck.cells["live_demo"].hide_def is True
...
def test_set_hide_def_does_not_affect_other_cells(tmp_path):
    ...
    kernel.set_hide_def(session, "setup", True)
    kernel.set_hide_def(session, "live_demo", True)

    assert kernel.deck.cells["setup"].hide_def is True
    assert kernel.deck.cells["live_demo"].hide_def is True


def test_set_hide_def_without_a_deck_path_raises():
    app = App()
    ...
    with pytest.raises(ValueError, match="not started from a deck file"):
        kernel.set_hide_def(session, "one", True)
